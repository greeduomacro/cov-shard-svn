...
{
	public class AOLContractGump : Gump
	{
		private AOLContract MCparent;
		
		public AOLContractGump( Mobile from, AOLContract parentMC ) : base( 0, 0 )
		{
			from.CloseGump( typeof( AOLContractGump ) );
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( info.ButtonID == 2 )
			{
				...
				m_from.AddToBackpack( new AOLClue() );
				m_from.AddToBackpack( new CharlesMarker2());
			}
		}
		...
		private class MonsterCorpseTarget : Target
		{
			...
			public MonsterCorpseTarget( AOLContract parentMC ) : base( -1, true, TargetFlags.None )
			{
				MCparent = parentMC;
			}
