...
{
	public class EvilSaraEgg : Item, ICarvable
	{
		private EvilSaraTimer m_Timer;

		[Constructable]
		public EvilSaraEgg() : base( 0xC6A )
		{
			Movable = false;
			Name = "an Explosive Egg";

			m_Timer = new EvilSaraTimer( this );
			m_Timer.Start();
		}
		...
		public EvilSaraEgg( Serial serial ) : base( serial )
		{
		}
		...
		private class EvilSaraTimer : Timer
		{
			private Item m_Item;

			public EvilSaraTimer( Item item ) : base( TimeSpan.FromSeconds( Utility.RandomMinMax( 2, 6 ) ) )
			{
