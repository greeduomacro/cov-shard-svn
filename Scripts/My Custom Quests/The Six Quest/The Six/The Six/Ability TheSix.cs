...
namespace Server
{
	public partial class Ability
	{
		...
		public class SimpleFlameTimer : Timer
		{
			...
			public SimpleFlameTimer( Mobile from, Mobile target ) : base( TimeSpan.FromSeconds( 1.0 ), TimeSpan.FromSeconds( 1.0 ) )
			{
				m_From = from;
				m_Target = target;
				m_Count = 0;
						...
						for ( int j = -2; j < 5; j++ )
							if ( (i == -2 || i == 2) || (j == -2 || j == 2) )
								Effects.SendMovingParticles( 
									new Entity( Serial.Zero, new Point3D( m_From.X + i, m_From.Y + j, m_From.Z + 5 ), m_From.Map ), 
									m_Target, 0x36D4, 10, 0, false, false, 0, 0, 9502, 1, 0, (EffectLayer)255, 0x100 );
							else
								continue;
				}
				else
		...
		public class SoulDrainTimer : Timer
		{
			...
			public SoulDrainTimer( Mobile from, List<PlayerMobile> list ) : base( TimeSpan.FromSeconds( 2.0 ), TimeSpan.FromSeconds( 2.0 ) )
			{
				m_From = from;
                //m_Damage = damage;
				m_List = list;
        ...
        public class OtherFireFieldItem2 : FireFieldSpell.FireFieldItem
        {
            public override bool BlocksFit { get { return false; } }

            public OtherFireFieldItem2(int itemID, Point3D loc, Mobile caster, Map map, TimeSpan duration, int val, int damage)
                : base(itemID, loc, caster, map, duration, val, damage)
            {
            }

            public OtherFireFieldItem2(Serial serial)
                : base(serial)
            {
            }
...
namespace Server.Commands
{
    public partial class TheSixCommands
    {
        public static void Initialize()
        {
            ...
            CommandSystem.Register("FlameCross2", AccessLevel.Seer, new CommandEventHandler(FlameCross2_OnCommand)); 
        ...
        public static void SimpleFlame_CallBack(Mobile from, object targeted)
        {
            if (targeted is Mobile)
                Ability.SimpleFlame(from, (Mobile)targeted);
            else
        ...
        [Description("Use the Six's Fire Field attack")]
        public static void FlameCross2_OnCommand(CommandEventArgs e)
        {
            Ability.FlameCross2(e.Mobile);
        }
