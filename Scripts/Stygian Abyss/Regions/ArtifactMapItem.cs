...
namespace Server.Items
{
	...
	public class ArtifactMapItem : MapItem, ICraftable
	{
		...
		[Constructable]
		public ArtifactMapItem() : base( 0x14EC )
		{
			Weight = 1.0;
		...
		public ArtifactMapItem( Serial serial ) : base( serial )
		{
		}
		...
		private static void OnMapCommand( NetState state, PacketReader pvSrc )
		{
			...
			ArtifactMapItem map = World.FindItem( pvSrc.ReadInt32() ) as ArtifactMapItem;
		...
		{
			public MapDetails( ArtifactMapItem map ) : base ( 0x90, 19 )
			{
		...
		{
			public NewMapDetails( ArtifactMapItem map ) : base ( 0xF5, 21 )
			{
				m_Stream.Write( (int) map.Serial );
				m_Stream.Write( (short) 0x139D );
				m_Stream.Write( (short) map.Bounds.Start.X );
		...
		{
			public MapCommand( ArtifactMapItem map, int command, int number, int x, int y ) : base ( 0x56, 11 )
			{
				m_Stream.Write( (int) map.Serial );
		...
		{
			public MapDisplay( ArtifactMapItem map ) : base( map, 5, 0, 0, 0 )
			{
			}
		}
		...
		{
			public MapAddPin( ArtifactMapItem map, Point2D point ) : base( map, 1, 0, point.X, point.Y )
			{
			}
		}
		...
		{
			public MapSetEditable( ArtifactMapItem map, bool editable ) : base( map, 7, editable ? 1 : 0, 0, 0 )
			{
			}
		}
