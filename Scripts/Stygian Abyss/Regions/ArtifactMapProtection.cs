...
namespace Server
{
	public class ArtifactRegion : BaseRegion
	{
		private const int Range = 5; // No house may be placed within 5 tiles of the treasure

		public ArtifactRegion( int x, int y, Map map ): base( null, map, Region.DefaultPriority, new Rectangle2D( x - Range, y - Range, 1 + (Range * 2), 1 + (Range * 2) ) )
		{
				...
				using ( StreamReader ip = new StreamReader( filePath ) )
				{
					...
					while ( (line = ip.ReadLine()) != null )
					{
						...
						try
						{
							...
							try
							{
								new ArtifactRegion( x, y, Map.Tokuno );
							}
							catch ( Exception e )
							{
						...
						{
                            Console.WriteLine( "Warning: Error in Line '{0}' of Data/artifact.cfg", line );
						}
					}
				}
			}
