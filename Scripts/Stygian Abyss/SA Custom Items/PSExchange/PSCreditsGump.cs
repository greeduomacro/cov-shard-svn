...
namespace Server.Gumps
{
    public class PSCreditsGump : Gump
    {
        ...
        public PSCreditsGump() : base( 0, 0 )
        {
			...
			AddLabel(104, 123, 0, @"110 Powerscroll (1000 Credits)");
			AddLabel(105, 157, 0, @"115 Powerscroll (7500 Credits)");
			AddLabel(104, 193, 0, @"120 Powerscroll (10000 Credits)");
			AddLabel(23, 348, 0, @"By:  Tannis Elohim");
        ...
        public override void OnResponse(NetState sender, RelayInfo info)
        {
            ...
            switch(info.ButtonID)
            {
				...
				{
					Item[] Token = from.Backpack.FindItemsByType( typeof( PSCredits ) );
					if ( from.Backpack.ConsumeTotal( typeof( PSCredits ), 7500 ) )
						{
							PowerScroll ps = CreateRandomPowerScroll();
							ps.Value = 115;
							from.AddToBackpack( ps );
				...
				{
					Item[] Token = from.Backpack.FindItemsByType( typeof( PSCredits ) );
					if ( from.Backpack.ConsumeTotal( typeof( PSCredits ), 10000 ) )
						{
							PowerScroll ps = CreateRandomPowerScroll();
							ps.Value = 120;
							from.AddToBackpack( ps );
