    ...
    public class IronBeetle : BaseCreature
    {
        ...
        public bool DoDisposeOre(int x, int y, int z, Map map, Mobile from)
        {
            ...
            if (ore.Resource > CraftResource.Iron)
            {
                ...
                if (this.Skills.Mining.Base < 100.0) this.Say("skill = {0}", Convert.ToString(skillbase));
                switch (ore.Resource)
                {
                ...
                if (Utility.RandomDouble() <= 0.01 || skillbase < minskill) { ore = new IronOre(); minskill = 00.0; minskill2 = 00.0; maxskill = 100.0; }
