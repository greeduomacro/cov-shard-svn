...
{
	[CorpseName( "a CuSidhe corpse" )]
	public class CuSidhe : BaseMount
	{
		[Constructable]
		public CuSidhe() : this( "a CuSidhe" )
		{
		}
		...
		{			
            ...
            if (chance <= 1)
                ...
                Hue = Utility.RandomList(0x481);
            else if (chance < 50)
                Hue = Utility.RandomList(0x657, 0x515, 0x4B1, 0x480, 0x455);
            else if (chance < 500)
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( from.Race != Race.Elf && from == ControlMaster && from.AccessLevel == AccessLevel.Player )
			{
				...
				if ( pads is PadsOfTheCuSidhe )
					from.SendMessage( "Your boots allow you to mount the CuSidhe", null, 0x3B2 );
				else
				{
