...
namespace Server.Customs.Galleons
{
    ...
    public abstract class BaseGalleon : BaseMulti
    {
        ...
        public static BaseGalleon FindGalleonAt(IPoint2D loc, Map map)
        {
            ...
            {
                BaseGalleon boat = sector.Multis[i] as BaseGalleon;
        ...
        private List<BoardRope> m_Ropes;
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public Mobile TillerMan { get { return m_TillerMan; } set { m_TillerMan = value; } }

        [CommandProperty(AccessLevel.GameMaster)]
        public List<BoardRope> Ropes { get { return m_Ropes; } set { m_Ropes = value; } }
        ...
        public virtual int HoldOffset { get { return 0; } }
        public virtual int TillerManOffset { get { return 0; } }
        ...
        private static List<BaseGalleon> m_Instances = new List<BaseGalleon>();

        public static List<BaseGalleon> Boats { get { return m_Instances; } }
            ...
            : base(0x0)
        {
            ...
            if (m_Ropes == null)
                m_Ropes = new List<BoardRope>(); //must be added at Galleon ctor
        ...
        {
            if (m_Ropes != null)
                foreach (BoardRope b in m_Ropes)
                {
                    b.MoveToWorld(GetRotatedLocation(PortOffset.X, PortOffset.Y), Map);
                }
            else
                m_Ropes = new List<BoardRope>();

            int xOffset = 0, yOffset = 0, zOffset = 0;
            Offset(m_Facing, ref xOffset, ref yOffset);

            if (m_TillerMan != null)
            {
                m_TillerMan.Location = new Point3D(X + (xOffset * TillerManOffset) + (m_Facing == Direction.North ? 1 : 0), Y + (yOffset * TillerManOffset), Location.Z + 18);
                m_TillerMan.SetDirection(m_Facing);
                m_TillerMan.InvalidateProperties();
            }

            if (m_Hold != null)
            {
                m_Hold.Location = new Point3D(X + (xOffset * HoldOffset), Y + (yOffset * HoldOffset), Location.Z + 18);
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            for (int i = 0; i < m_Ropes.Count; i++) writer.Write((Item)m_Ropes[i]);
                ...
                case 4:
                    {
                        ...
                        for (int i = 0; i < toread; i++)
                            m_Ropes.Add(reader.ReadItem() as BoardRope);
        ...
        public override void OnAfterDelete()
        {
            ...
            foreach (BoardRope b in m_Ropes) b.Delete();
        ...
        public override void OnLocationChange(Point3D old)
        {
            ...
            foreach (BoardRope b in m_Ropes)
                b.Location = new Point3D(X + (b.X - old.X), Y + (b.Y - old.Y), Z + (b.Z - old.Z));
        }
        ...
        private class DecayTimer : Timer
        {
            ...
            public DecayTimer(BaseGalleon boat)
        ...
        public class RenameBoatPrompt : Server.Prompts.Prompt
        {
            private BaseGalleon m_Boat;

            public RenameBoatPrompt(BaseGalleon galleon)
            {
                m_Boat = galleon;
            }
        ...
        public DryDockResult CheckDryDock(Mobile from)
        {
            ...
            {
                if (o == this || o == m_Hold || o is BoardRope || o == m_TillerMan)
                    continue;
        ...
        private class TurnTimer : Timer
        {
            ...
            public TurnTimer(BaseGalleon boat, int offset)
        ...
        public void Teleport(int xOffset, int yOffset, int zOffset)
        {
            ...
            eable = this.Map.GetObjectsInBounds(new Rectangle2D(X + mcl.Min.X, Y + mcl.Min.Y, mcl.Width, mcl.Height));
            ...
            foreach (object o in tr)
                toMove.Remove(o);

            eable = this.Map.GetMobilesInRange(Location, 17);
            foreach (object o in eable)
            {
                if (o != null)
                {
                    ...
                    if (o is PlayerMobile)
                        mob.Send(new SmoothMove(this, toMove.ToArray()));
        ...
        private class SmoothMove : Packet
        {
        ...
        public bool SetFacing(Direction facing)
        {
            ...
            if (m_TillerMan != null)
                m_TillerMan.SetDirection(facing);
            ...
            foreach (object o in eable)
            {
                if (o is Item)
                {
                    ...
                    if (item != this && Contains(item) && item.Visible && item.Z >= Z)
                        if (!(item is Hold))
                            toMove.Add(item);
                }
                else if ((o is Mobile && Contains((Mobile)o)) && !(o is TillerMan))
                {
                    toMove.Add(o);
            ...
            Offset(facing, ref xOffset, ref yOffset);
            ...
            if (m_TillerMan != null)
            {
                //m_TillerMan.Location = Rotate(m_TillerMan.Location, count); //new Point3D(X + (xOffset * TillerManOffset.X), Y + (yOffset * TillerManOffset.Y), Location.Z + TillerManOffset.Z);
                //m_TillerMan.Location = new Point3D(m_TillerMan.X, m_TillerMan.Y, Location.Z + 18);
                m_TillerMan.Location = GetRotatedLocation((X - m_TillerMan.X), (Y - m_TillerMan.Y));
            }

            if (m_Hold != null)
            {
                //m_Hold.Location = Rotate(m_TillerMan.Location, count); //new Point3D(X + (xOffset * HoldDistance), Y + (yOffset * HoldDistance), m_Hold.Z);
                //m_Hold.Location = new Point3D(m_Hold.X, m_Hold.Y, Location.Z + HoldOffset.Z);
        ...
        private class MoveTimer : Timer
        {
            ...
            public MoveTimer(BaseGalleon boat, TimeSpan interval, bool single)
