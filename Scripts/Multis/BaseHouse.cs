		...
		public void MoveAllToCrate()
		{
			...
			foreach ( SecureInfo info in Secures )
			{
				...
				if ( !item.Deleted )
				{
					...
					if ( item.Parent == null )
						DropToMovingCrate( item );
//Added for fountain of life
					if ( item is BaseAddonContainer )
					{
						DropToMovingCrate( ((IAddon)item).Deed );
						item.Delete();
					}
//End add for fountain of life
				}
			}
...
//Added for fountain of life
						if ( !info.Item.Deleted && info.Item is BaseAddonContainer )
						{
							Item containerDeed = ((IAddon)info.Item).Deed;
							if ( containerDeed != null )
								containerDeed.MoveToWorld( info.Item.Location, info.Item.Map );
							info.Item.Delete();
						}
//End add for fountain of life
					}
