		...
		public void EndCombine( Mobile from, object o )
		{
			if ( o is Item && ((Item)o).IsChildOf( from.Backpack ) )
			{
				if ( o is SmallBOD )
				{
					...
					else if ( m_Material >= BulkMaterialType.DullCopper && m_Material <= BulkMaterialType.Sunstone && small.Material != m_Material )
					{
						from.SendLocalizedMessage( 1045162 ); // Both orders must use the same ore type.
					}
					else if ( m_Material >= BulkMaterialType.Spined && m_Material <= BulkMaterialType.Daemon && small.Material != m_Material )
					{
						from.SendLocalizedMessage( 1049351 ); // Both orders must use the same leather type.
					}
					else if ( m_Material >= BulkMaterialType.Pine && m_Material <= BulkMaterialType.Oak && small.Material != m_Material )
					{
