...
namespace Server.Misc
{
	...
	{
		private static bool m_Enabled = (Core.Expansion == Expansion.HS);
		...
		private static Type[] m_LesserArtifacts = new Type[]
			{
				...
				typeof( TomeOfEnlightenment ), typeof( AncientUrn ), typeof( HonorableSwords ), typeof( PigmentsOfTokuno ), typeof( FluteOfRenewal ), typeof(ChestOfHeirlooms)
			};
		...
		private static bool CheckLocation( Mobile m )
		{
			...
			if( r.IsPartOf( "Yomotsu Mines" ) || r.IsPartOf( "Fan Dancer's Dojo" ) || r.IsPartOf( "Isamu Dragon Valley" ) || r.IsPartOf( "Isamu Mt.Sho Castle") || r.IsPartOf( "Crane Marsh" ) )
				return true;
