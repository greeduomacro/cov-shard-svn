...
namespace Server.Mobiles
{
    ...
    public class BaseCreature : Mobile, IHonorTarget
    {
        ...
        private int m_IcyWindAttack;
        private int m_ShockAttack;
        ...
        [CommandProperty(AccessLevel.GameMaster)]
        public int IcyWindAttack
        {
            get { return m_IcyWindAttack; }
            set { m_IcyWindAttack = value; }
        }

        [CommandProperty(AccessLevel.GameMaster)]
        public int ShockAttack
        {
            get { return m_ShockAttack; }
            set { m_ShockAttack = value; }
        }
        ...
        public virtual void OnCarve(Mobile from, Corpse corpse, Item with)
        {
            ...
            else
            {
                ...
                if (fur != 0)
                {
                    corpse.AddCarvedItem(new Fur(fur), from);
                    from.SendMessage("You shear it, and the fur is now on the corpse.");
                }
               ...
               if (scales != 0)
				{
					...
					switch (sc)
					{
                        ...
                        case ScaleType.MedusaLight: corpse.AddCarvedItem(new MedusaLightScales(scales), from); break;
                        case ScaleType.MedusaDark: corpse.AddCarvedItem(new MedusaDarkScales(scales), from); break;
						case ScaleType.All:
							{
                                ...
                                corpse.AddCarvedItem(new MedusaLightScales(scales), from);
                                corpse.AddCarvedItem(new MedusaDarkScales(scales), from);
								break;
							}
					}                     
        ...
        public override void Serialize(GenericWriter writer)
        {
            ...
            writer.Write((int)m_IcyWindAttack);
            writer.Write((int)m_ShockAttack);
        ...
        public override bool OnBeforeDeath()
        {
            ...
            Mobile kill = this.LastKiller;
            if (kill is PlayerMobile)
            {
                PlayerMobile kill2 = kill as PlayerMobile;
                kill2.Exp += (long)(this.Fame / 100 + this.RawStr / 30 + this.RawDex / 20 + this.RawInt / 50 + this.HitsMax / 20 + (this.DamageMin + this.DamageMax) / 20 + (m_PhysicalResistance + m_FireResistance + m_ColdResistance + m_EnergyResistance + m_PoisonResistance) / 50 + 1) * ((int)kill2.Level / 10 + 1);
            }
            else if ((kill is BaseCreature) && !(kill is BaseGuard))
            {
                BaseCreature kill2 = kill as BaseCreature;
                if ((kill2.ControlMaster != null) && (kill2.ControlMaster is PlayerMobile))
                {
                    PlayerMobile kill3 = kill2.ControlMaster as PlayerMobile;
                    kill3.Exp += (long)((this.Fame / 200 + this.RawStr / 75 + this.RawDex / 60 + this.RawInt / 100 + this.HitsMax / 60 + (this.DamageMin + this.DamageMax) / 20 + (m_PhysicalResistance + m_FireResistance + m_ColdResistance + m_EnergyResistance + m_PoisonResistance) / 50) + 1);
                }
            }
       ...
       /* public void PackArcaneScroll(int max)
            ...
            else
            {
                if (!Summoned && !m_NoKillAwards)
                {
                    ...
                    for (int i = 0; i < list.Count; ++i)
                    {
                        ...
                        //COVCASHValidate.COVCashAward(ds.m_Mobile, this);
        ...
        public virtual void OnThink()
        {
            ...
            if (DateTime.Now >= m_NextBreathTime) // tested: controled dragons do breath fire, what about summoned skeletal dragons?
            {
                ...
                if (target != null && target.Alive && !target.IsDeadBondedPet && CanBeHarmful(target) && target.Map == this.Map && !IsDeadBondedPet && target.InRange(this, BreathRange) && InLOS(target) && !BardPacified)
                {
                    if (CanBreath)
                        BreathStart(target);
                    else //if ( CanBreath && this.Controlled == true && this.ControlMaster != null )
                        PetMoves.DoMoves(this, target);
                }
            ...
            if (EnableMasterHeal) HealControlMaster();
        }

        public virtual bool EnableMasterHeal { get { return (false); } }

        private DateTime lastused = DateTime.Now;
        private TimeSpan delay = TimeSpan.FromSeconds(Utility.RandomMinMax(10, 20));
        private Mobile owner;
        private int ownerhits;

        public void HealControlMaster()
        {
            if (lastused + delay > DateTime.Now) return;
            else lastused = DateTime.Now;

            int thishits = this.Hits;
            if (this.Controlled == true && this.ControlMaster != null && this.Alive == true && this.Mana > 0)
            {
                owner = this.ControlMaster;
                ownerhits = owner.Hits;
            }
            else return;
            if (ownerhits < owner.HitsMax && this.CanSee(owner) && owner.Criminal == false) //G Heal Code
            {
                if (owner.Alive)
                {
                    if (owner.Poisoned == false)
                    {
                        owner.SendMessage("Your pet sees you are injured, and heals you");
                        owner.FixedParticles(0x376A, 9, 32, 5030, EffectLayer.Waist);
                        owner.PlaySound(0x202);
                        int healed = (int)(this.Skills[SkillName.Magery].Value * 0.4);
                        owner.Hits += healed;
                        this.Mana -= 15;
                        return;
                    }
                    ...
                    {
                        owner.SendMessage("Your pet senses you are ill");
                        owner.FixedParticles(0x373A, 10, 15, 5012, EffectLayer.Waist);
                        owner.PlaySound(0x1E0);
                        owner.SendMessage("Your pet has cured you of all poisons.");
                        owner.CurePoison(this);
                        this.Mana -= 10;
                        return;
                    }
                }
                ...
                {
                    if (owner.InRange(this.Location, 2)) //Res Code
                    {
                        double chance = (this.Skills[SkillName.Magery].Value - 50) / 100;

                        if (chance >= Utility.RandomDouble() && this.Skills[SkillName.Magery].Value >= 100.0)
                        {
                            owner.PlaySound(0x214);
                            owner.FixedEffect(0x376A, 10, 16);
                            owner.CloseGump(typeof(Server.Gumps.ResurrectGump));
                            owner.SendGump(new Server.Gumps.ResurrectGump(owner, this));
                            return;
                        }
                        else
                        {
                            owner.SendMessage("Your pet failed to resurect you.");
                            return;
                        }
                    }
                    else
                    {
                        owner.SendMessage("Your pet attempted to resurect you, but you were not close enough.");
                        return;
                    }
                    //return;
                }
            ...
            else
            {
                ...
                if (this.Hits < this.HitsMax && this.Poisoned == false)
                {
