...
namespace Server.Gumps
{
	public class FindHouseGump : Gump
	{
		...
		public FindHouseGump( Mobile from, ArrayList list, int page ) : base( 50, 40 )
		{
			...
			AddLabelCropped( 182, 16, 120, 20, 1152, "Location" );
			AddLabel( 80, 290, 93, String.Format( "Haazen's House Locator      {0} Houses are in the world", Houses ));
		...
		private void AddDetails( int index )
		{
			if ( index < m_List.Count )
			{
				...
				//AddLabel(32, 40 +(row * 20), 1152, String.Format( "{0}", House.ShipName ));
				AddLabel(32, 40 +(row * 20), 1152, String.Format( "{0}", House.Owner ));
				AddLabel(182, 40 +(row * 20), 1152, String.Format( "{0} {1}", House.GetWorldLocation(), House.Map));
