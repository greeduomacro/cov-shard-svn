...
{
	public class RuneLibraryDungeonsFelucca : Item
	{
		private static string pathlist = "Data/RuneLibraryDungeons.txt";
      ...
      {
         CommandSystem.Register( "RuneLibraryDungeonsFelucca", AccessLevel.Owner, new CommandEventHandler( RuneLibraryDungeonsFelucca_OnCommand ) );
      }
		...
		[Constructable]
		public RuneLibraryDungeonsFelucca (Mobile from)
		{
			...
			for( int i=0; i<size; i++ )
			{
				...
				{
					rb.Name = "Dungeons Felucca " + nameStart + " - " + (nameEnd-1);
					library.Add(rb);
					rb = new Runebook(0);
					nameStart = nameEnd;
				}
		...
		public RuneLibraryDungeonsFelucca( Serial serial ) : base( serial )
		{
		}
