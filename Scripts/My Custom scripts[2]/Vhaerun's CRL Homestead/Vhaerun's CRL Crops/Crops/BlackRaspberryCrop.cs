...
namespace Server.Items.Crops
{
	...
	{ 
		...
		[Constructable]
		public BlackRaspberryTreeSeed() : this( 1 )
		{
		}

		[Constructable]
		public BlackRaspberryTreeSeed( int amount ) : base( 0xF27 )
		{
			Stackable = true; 
			Weight = .5; 
			Hue = 0x5E2; 
		...
		public BlackRaspberryTreeSeed( Serial serial ) : base( serial ) 
		{ 
		} 
	...
	{ 
		...
		[Constructable] 
		public BlackRaspberrySapling() : base( Utility.RandomList ( 0xCC9, 0xCC9 ) ) 
		{ 
			Movable = false; 
		...
		public BlackRaspberrySapling( Serial serial ) : base( serial ) 
		{ 
		} 
	...
	public class BlackRaspberryTree : BaseTree
	{
		...
		[Constructable] 
		public BlackRaspberryTree( Point3D pnt, Map map ) : base( 0xC9E ) // leaves
		{
			Movable = false;
			MoveToWorld( pnt, map );
			Hue = 0x298;
		...
		private class FruitTimer : Timer
		{
			...
			public FruitTimer( BlackRaspberryTree plant ) : base( TimeSpan.FromSeconds( 900 ), TimeSpan.FromSeconds( 30 ) )
			{
		...
		public void Chop( Mobile from )
		{
			...
			{ 
				if ( ( chopTimer == null ) || ( !chopTimer.Running ) )
				{
					...
					{
						BlackRaspberry fruit = new BlackRaspberry( (int)Utility.Random( 13 ) + m_yield );
		...
		private class StumpTimer : Timer
		{
			...
			public StumpTimer( BlackRaspberryTree Tree, FruitTreeStump Stump, Mobile from ) : base( TimeSpan.FromMilliseconds( 5500 ) )
			{
		...
		public BlackRaspberryTree( Serial serial ) : base( serial )
		{
		}
