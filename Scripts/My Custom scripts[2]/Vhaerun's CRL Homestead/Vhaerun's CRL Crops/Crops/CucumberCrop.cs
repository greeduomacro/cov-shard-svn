...
namespace Server.Items.Crops
{
	...
	{ 
		...
		[Constructable] 
		public CucumberSeedling() : base( 0xC5E ) 
		{ 
			Movable = false; 
	...
	public class CucumberCrop : BaseTree
	{
		...
		[Constructable] 
		public CucumberCrop( Point3D pnt, Map map ) : base( 0xC69 ) // leaves
		{
			Movable = false;
			MoveToWorld( pnt, map );
			Name = "Cucumber Plant";
		...
		private class FruitTimer : Timer
		{
			...
			public FruitTimer( CucumberCrop plant ) : base( TimeSpan.FromSeconds( 900 ), TimeSpan.FromSeconds( 30 ) )
			{
		...
		private class StumpTimer : Timer
		{
			...
			public StumpTimer( CucumberCrop Tree, FruitTreeStump Stump, Mobile from ) : base( TimeSpan.FromMilliseconds( 5500 ) )
			{
