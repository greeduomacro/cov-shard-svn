...
namespace Server.Items.Crops
{
	...
	{ 
		...
		[Constructable]
		public CranberryTreeSeed() : this( 1 )
		{
		}

		[Constructable]
		public CranberryTreeSeed( int amount ) : base( 0xF27 )
		{
			Stackable = true; 
			Weight = .5; 
			Hue = 0x5E2; 
		...
		public CranberryTreeSeed( Serial serial ) : base( serial ) 
		{ 
		} 
	...
	{ 
		...
		[Constructable] 
		public CranberrySapling() : base( Utility.RandomList ( 0xCC9, 0xCC9 ) ) 
		{ 
			Movable = false; 
		...
		public CranberrySapling( Serial serial ) : base( serial ) 
		{ 
		} 
	...
	public class CranberryTree : BaseTree
	{
		...
		[Constructable] 
		public CranberryTree( Point3D pnt, Map map ) : base( 0xC9E ) // leaves
		{
			Movable = false;
			MoveToWorld( pnt, map );
			Hue = 0x4B;
			...
			if ( DateTime.Now > lastpicked.AddSeconds(3) ) // 3 seconds between picking
			{
				...
				{ 
					...
					else //check skill
					{
						...
						m_yield -= pick;
						from.SendMessage( "You pick {0} Cranberry{1}!", pick, ( pick == 1 ? "" : "s" ) ); 
		...
		private class FruitTimer : Timer
		{
			...
			public FruitTimer( CranberryTree plant ) : base( TimeSpan.FromSeconds( 900 ), TimeSpan.FromSeconds( 30 ) )
			{
		...
		private class StumpTimer : Timer
		{
			...
			public StumpTimer( CranberryTree Tree, FruitTreeStump Stump, Mobile from ) : base( TimeSpan.FromMilliseconds( 5500 ) )
			{
		...
		public CranberryTree( Serial serial ) : base( serial )
		{
		}
