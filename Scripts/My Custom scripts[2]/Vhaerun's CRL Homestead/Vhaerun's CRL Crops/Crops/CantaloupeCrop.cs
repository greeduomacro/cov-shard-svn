...
namespace Server.Items.Crops
{
	...
	{ 
		...
		[Constructable] 
		public CantaloupeSeedling() : base( 0xC5E ) 
		{ 
			Movable = false; 
	...
	public class CantaloupeCrop : BaseTree
	{
		...
		[Constructable] 
		public CantaloupeCrop( Point3D pnt, Map map ) : base( Utility.RandomList( 0xC5F, 0xC60 ) ) // leaves
		{
			Movable = false;
			MoveToWorld( pnt, map );
			...
			if ( DateTime.Now > lastpicked.AddSeconds(3) ) // 3 seconds between picking
			{
				...
				{ 
					...
					else //check skill
					{
						...
						m_yield -= pick;
						from.SendMessage( "You pick {0} Cantaloupe{1}!", pick, ( pick == 1 ? "" : "s" ) ); 
		...
		private class FruitTimer : Timer
		{
			...
			public FruitTimer( CantaloupeCrop plant ) : base( TimeSpan.FromSeconds( 900 ), TimeSpan.FromSeconds( 30 ) )
			{
		...
		private class StumpTimer : Timer
		{
			...
			public StumpTimer( CantaloupeCrop Tree, FruitTreeStump Stump, Mobile from ) : base( TimeSpan.FromMilliseconds( 5500 ) )
			{
