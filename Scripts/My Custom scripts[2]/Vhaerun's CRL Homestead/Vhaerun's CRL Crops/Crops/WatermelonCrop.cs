...
namespace Server.Items.Crops
{
	...
	{ 
		...
		[Constructable] 
		public WatermelonSeedling() : base( 0xC5E ) 
		{ 
			Movable = false; 
	...
	public class WatermelonCrop : BaseTree
	{
		...
		[Constructable] 
		public WatermelonCrop( Point3D pnt, Map map ) : base( Utility.RandomList( 0xC5F, 0xC60 ) ) // leaves
		{
			Movable = false;
			MoveToWorld( pnt, map );
			...
			if ( DateTime.Now > lastpicked.AddSeconds(3) ) // 3 seconds between picking
			{
				...
				{ 
					...
					else //check skill
					{
						...
						m_yield -= pick;
						from.SendMessage( "You pick {0} Watermelon{1}!", pick, ( pick == 1 ? "" : "s" ) ); 
		...
		private class FruitTimer : Timer
		{
			...
			public FruitTimer( WatermelonCrop plant ) : base( TimeSpan.FromSeconds( 900 ), TimeSpan.FromSeconds( 30 ) )
			{
		...
		private class StumpTimer : Timer
		{
			...
			public StumpTimer( WatermelonCrop Tree, FruitTreeStump Stump, Mobile from ) : base( TimeSpan.FromMilliseconds( 5500 ) )
			{
