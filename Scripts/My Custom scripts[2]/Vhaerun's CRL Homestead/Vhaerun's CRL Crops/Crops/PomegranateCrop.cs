...
namespace Server.Items.Crops
{
	...
	{ 
		...
		[Constructable]
		public PomegranateTreeSeed() : this( 1 )
		{
		}

		[Constructable]
		public PomegranateTreeSeed( int amount ) : base( 0xF27 )
		{
			Stackable = true; 
			Weight = .5; 
			Hue = 0x5E2; 
		...
		public PomegranateTreeSeed( Serial serial ) : base( serial ) 
		{ 
		} 
	...
	{ 
		...
		[Constructable] 
		public PomegranateSapling() : base( Utility.RandomList ( 0xCE9, 0xCEA ) ) 
		{ 
			Movable = false; 
		...
		public PomegranateSapling( Serial serial ) : base( serial ) 
		{ 
		} 
	...
	public class PomegranateTree : BaseTree
	{
		...
		[Constructable] 
		public PomegranateTree( Point3D pnt, Map map ) : base( 0xD9A ) // leaves
		{
			Movable = false;
			MoveToWorld( pnt, map );
			...
			if ( DateTime.Now > lastpicked.AddSeconds(3) ) // 3 seconds between picking
			{
				...
				{ 
					...
					else //check skill
					{
						...
						m_yield -= pick;
						from.SendMessage( "You pick {0} Pomegranate{1}!", pick, ( pick == 1 ? "" : "s" ) ); 
		...
		private class FruitTimer : Timer
		{
			...
			public FruitTimer( PomegranateTree plant ) : base( TimeSpan.FromSeconds( 900 ), TimeSpan.FromSeconds( 30 ) )
			{
		...
		private class StumpTimer : Timer
		{
			...
			public StumpTimer( PomegranateTree Tree, FruitTreeStump Stump, Mobile from ) : base( TimeSpan.FromMilliseconds( 5500 ) )
			{
		...
		public PomegranateTree( Serial serial ) : base( serial )
		{
		}
