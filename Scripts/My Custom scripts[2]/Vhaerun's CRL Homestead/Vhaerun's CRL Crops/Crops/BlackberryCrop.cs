...
namespace Server.Items.Crops
{
	...
	{ 
		...
		[Constructable]
		public BlackberryTreeSeed() : this( 1 )
		{
		}

		[Constructable]
		public BlackberryTreeSeed( int amount ) : base( 0xF27 )
		{
			Stackable = true; 
			Weight = .5; 
			Hue = 0x5E2; 
		...
		public BlackberryTreeSeed( Serial serial ) : base( serial ) 
		{ 
		} 
	...
	{ 
		...
		[Constructable] 
		public BlackberrySapling() : base( Utility.RandomList ( 0xCC9, 0xCC9 ) ) 
		{ 
			Movable = false; 
		...
		public BlackberrySapling( Serial serial ) : base( serial ) 
		{ 
		} 
	...
	public class BlackberryTree : BaseTree
	{
		...
		[Constructable] 
		public BlackberryTree( Point3D pnt, Map map ) : base( 0xC9E ) // leaves
		{
			Movable = false;
			MoveToWorld( pnt, map );
			Hue = 0x257;
			...
			if ( DateTime.Now > lastpicked.AddSeconds(3) ) // 3 seconds between picking
			{
				...
				{ 
					...
					else //check skill
					{
						...
						m_yield -= pick;
						from.SendMessage( "You pick {0} Blackberry{1}!", pick, ( pick == 1 ? "" : "s" ) ); 
		...
		private class FruitTimer : Timer
		{
			...
			public FruitTimer( BlackberryTree plant ) : base( TimeSpan.FromSeconds( 900 ), TimeSpan.FromSeconds( 30 ) )
			{
		...
		private class StumpTimer : Timer
		{
			...
			public StumpTimer( BlackberryTree Tree, FruitTreeStump Stump, Mobile from ) : base( TimeSpan.FromMilliseconds( 5500 ) )
			{
		...
		public BlackberryTree( Serial serial ) : base( serial )
		{
		}
