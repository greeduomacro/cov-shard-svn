...
namespace Server.Items.Crops
{
	...
	{ 
		...
		[Constructable]
		public RedRaspberryTreeSeed() : this( 1 )
		{
		}

		[Constructable]
		public RedRaspberryTreeSeed( int amount ) : base( 0xF27 )
		{
			Stackable = true; 
			Weight = .5; 
			Hue = 0x5E2; 
		...
		public RedRaspberryTreeSeed( Serial serial ) : base( serial ) 
		{ 
		} 
	...
	{ 
		...
		[Constructable] 
		public RedRaspberrySapling() : base( Utility.RandomList ( 0xCC9, 0xCC9 ) ) 
		{ 
			Movable = false; 
		...
		public RedRaspberrySapling( Serial serial ) : base( serial ) 
		{ 
		} 
	...
	public class RedRaspberryTree : BaseTree
	{
		...
		[Constructable] 
		public RedRaspberryTree( Point3D pnt, Map map ) : base( 0xC9E ) // leaves
		{
			Movable = false;
			MoveToWorld( pnt, map );
			Hue = 0x2DE;
			...
			if ( DateTime.Now > lastpicked.AddSeconds(3) ) // 3 seconds between picking
			{
				...
				{ 
					...
					else //check skill
					{
						...
						m_yield -= pick;
						from.SendMessage( "You pick {0} RedRaspberry{1}!", pick, ( pick == 1 ? "" : "s" ) ); 
		...
		private class FruitTimer : Timer
		{
			...
			public FruitTimer( RedRaspberryTree plant ) : base( TimeSpan.FromSeconds( 900 ), TimeSpan.FromSeconds( 30 ) )
			{
		...
		private class StumpTimer : Timer
		{
			...
			public StumpTimer( RedRaspberryTree Tree, FruitTreeStump Stump, Mobile from ) : base( TimeSpan.FromMilliseconds( 5500 ) )
			{
		...
		public RedRaspberryTree( Serial serial ) : base( serial )
		{
		}
