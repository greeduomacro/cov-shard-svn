...
{
	[CorpseName( "an Ancient CuSidhe corpse" )]
	public class AncientCuSidhe : BaseMount
	{
		[Constructable]
		public AncientCuSidhe() : this( "an Ancient CuSidhe" )
		{
		}
		
		[Constructable]
		public AncientCuSidhe( string name ) : base( name, 277, 0x3E91, AIType.AI_Animal, FightMode.Aggressor, 10, 1, 0.2, 0.4 )
		{			
			Hue = 2958;
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( from.Race != Race.Elf && from == ControlMaster && from.AccessLevel == AccessLevel.Player )
			{
				...
				if ( pads is PadsOfTheCuSidhe )
					from.SendMessage( "Your boots allow you to mount the  Ancient CuSidhe", null, 0x3B2 );
				else
				{
		...
		public AncientCuSidhe( Serial serial ) : base( serial )
		{
		}
