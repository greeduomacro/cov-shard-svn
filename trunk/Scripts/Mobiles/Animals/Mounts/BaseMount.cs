...
namespace Server.Mobiles
{
	public abstract class BaseMount : BaseCreature, IMount
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
            ...
            if (this is SilverSteed && from.Karma > 0)
            {
                from.SendMessage("Thou art not evil enough to ride this mount!");

                 Rider = null;
            }
            else if ( this is SilverSteed && from.Karma < 0)
            {
                from.SendMessage("Thou art Evil enough to ride this mount!");
