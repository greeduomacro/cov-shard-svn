...
{
	public class SilenceSpell : BlueSpell
	{
		private static SpellInfo m_Info = new SpellInfo( "Silence Spell", "", 230 );

		public override int PowerLevel{ get{ return 5; } }

		private Mobile m_Caster;

		public SilenceSpell( Mobile from ) : base( from, m_Info )
		{
			m_Caster = from;
		}
		...
		public static bool IsSilenceed( Mobile m )
		{
			return m_Table.ContainsKey( m.Serial );
		}

		public static void BeginSilence( Mobile m )
		{
			...
			m.SendMessage( "You have been silenced." );
		}

		public static void EndSilence( Mobile m )
		{
			if ( m_Table.ContainsKey( m.Serial ) )
			{
				...
				m_Table.Remove( m.Serial );
				m.SendMessage( "You are no longer silenced." );
			}
		}
		...
		{
			private SilenceSpell m_Owner;

			public InternalTarget( SilenceSpell owner ) : base( 12, false, TargetFlags.Harmful )
			{
				m_Owner = owner;
			}
