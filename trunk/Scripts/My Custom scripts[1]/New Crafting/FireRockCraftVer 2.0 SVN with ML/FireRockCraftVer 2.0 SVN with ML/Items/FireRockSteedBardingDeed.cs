...
namespace Server.Items
{
	...
	public class FireRockSteedBardingDeed : Item, ICraftable
	{
		...
		{
			get { return "a fire rock steed barding deed"; }
		}
		...
		public FireRockSteedBardingDeed() : base( 0x14F0 )
		{
			Weight = 1.0;
			Hue = 1161;
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( IsChildOf( from.Backpack ) )
			{
				...
				from.SendMessage( "Select the fire rock steed you wish to place the barding on." ); // Select the horse you wish to place the barding on.
			}
			else
			{
		...
		public virtual void OnTarget( Mobile from, object obj )
		{
			...
			FireRockSteed pet = obj as FireRockSteed;
			...
			{
				from.SendMessage( "You can only put barding on a tamed fire rock steed that you own." ); // You can only put barding on a tamed horse that you own.
			}
			...
			else
			{
				...
				from.SendMessage( "You place the barding on your fire rock steed.  The heated exterior of the beast causes the armor to meld to its skin, permanently!"); // You place the barding on your horse.  Use a bladed item on your horse to remove the armor.
			}
		}

		public FireRockSteedBardingDeed( Serial serial ) : base( serial )
		{
		}
