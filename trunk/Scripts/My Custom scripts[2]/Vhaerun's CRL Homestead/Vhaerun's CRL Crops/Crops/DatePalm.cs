...
{
	public class DatePalmSeed : BaseCrop 
	{ 
		...
		[Constructable]
		public DatePalmSeed() : this( 1 )
		{
		}

		[Constructable]
		public DatePalmSeed( int amount ) : base( 0xF27 )
		{
			Stackable = true; 
			Weight = .5; 
			Hue = 0x5E2; 
		...
		{ 
			...
			this.Consume(); 
			Item item = new DatePalmSapling(); 
			item.Location = m_pnt; 
			item.Map = m_map; 
			
		} 

		public DatePalmSeed( Serial serial ) : base( serial ) 
		{ 
		} 
	...
	public class DatePalmSapling : BaseCrop 
	{ 
		...
		[Constructable] 
		public DatePalmSapling() : base( Utility.RandomList ( 0xCE9, 0xCEA ) ) 
		{ 
			Movable = false; 
		...
		public static void init( DatePalmSapling plant )
		{
			TimeSpan delay = TreeHelper.SaplingTime;
			plant.treeTime = DateTime.Now + delay;

			plant.thisTimer = new TreeHelper.TreeTimer( plant, typeof(DatePalm), delay ); 
			plant.thisTimer.Start(); 
		}

		public DatePalmSapling( Serial serial ) : base( serial ) 
		{ 
		} 
	...
	public class DatePalm : BaseTree
	{
		public Item i_trunk;
		...
		[Constructable] 
		public DatePalm( Point3D pnt, Map map ) : base( Utility.RandomList( 0xD37, 0xD38 ) ) // leaves
		{
			Movable = false;
			MoveToWorld( pnt, map );
			Name = "Date Palm";
...
*/
		...
		public static void init ( DatePalm plant, bool full )
		{
...
*/
			if ( DateTime.Now > lastpicked.AddSeconds(3) ) // 3 seconds between picking
			{
				...
				{ 
					...
					else //check skill
					{
						...
						m_yield -= pick;
						from.SendMessage( "You pick {0} Date{1}!", pick, ( pick == 1 ? "" : "s" ) ); 
		...
		{
			private DatePalm i_plant;

			public FruitTimer( DatePalm plant ) : base( TimeSpan.FromSeconds( 900 ), TimeSpan.FromSeconds( 30 ) )
			{
		...
		public void Chop( Mobile from )
		{
			...
			{ 
				if ( ( chopTimer == null ) || ( !chopTimer.Running ) )
				{
					...
					{
						Dates fruit = new Dates( (int)Utility.Random( 5 ) + m_yield );
						...
						FruitTreeStump i_stump = new FruitTreeStump( typeof( DatePalm ) );
		...
		{
			private DatePalm i_tree;
			private FruitTreeStump i_stump;
			private Mobile m_chopper;

			public StumpTimer( DatePalm Tree, FruitTreeStump Stump, Mobile from ) : base( TimeSpan.FromMilliseconds( 5500 ) )
			{
		...
		public DatePalm( Serial serial ) : base( serial )
		{
		}
