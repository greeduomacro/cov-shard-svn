...
namespace Server.Items.Crops
{
	...
	{ 
		...
		[Constructable] 
		public PumpkinSeedling() : base( 0xC5E ) 
		{ 
			Movable = false; 
	...
	public class PumpkinCrop : BaseTree
	{
		...
		[Constructable] 
		public PumpkinCrop( Point3D pnt, Map map ) : base( Utility.RandomList( 0xC5F, 0xC60 ) ) // leaves
		{
			Movable = false;
			MoveToWorld( pnt, map );
			Name = "Pumpkin Plant";
		...
		private class FruitTimer : Timer
		{
			...
			public FruitTimer( PumpkinCrop plant ) : base( TimeSpan.FromSeconds( 900 ), TimeSpan.FromSeconds( 30 ) )
			{
		...
		private class StumpTimer : Timer
		{
			...
			public StumpTimer( PumpkinCrop Tree, FruitTreeStump Stump, Mobile from ) : base( TimeSpan.FromMilliseconds( 5500 ) )
			{
