...
namespace Server.Items.Crops
{
	...
	{ 
		...
		[Constructable]
		public GrapefruitTreeSeed() : this( 1 )
		{
		}

		[Constructable]
		public GrapefruitTreeSeed( int amount ) : base( 0xF27 )
		{
			Stackable = true; 
			Weight = .5; 
			Hue = 0x5E2; 
		...
		public GrapefruitTreeSeed( Serial serial ) : base( serial ) 
		{ 
		} 
	...
	{ 
		...
		[Constructable] 
		public GrapefruitSapling() : base( Utility.RandomList ( 0xCE9, 0xCEA ) ) 
		{ 
			Movable = false; 
		...
		public GrapefruitSapling( Serial serial ) : base( serial ) 
		{ 
		} 
	...
	public class GrapefruitTree : BaseTree
	{
		...
		[Constructable] 
		public GrapefruitTree( Point3D pnt, Map map ) : base( Utility.RandomList( 0xD9E, 0xDA2 ) ) // leaves
		{
			Movable = false;
			MoveToWorld( pnt, map );
			...
			if ( DateTime.Now > lastpicked.AddSeconds(3) ) // 3 seconds between picking
			{
				...
				{ 
					...
					else //check skill
					{
						...
						m_yield -= pick;
						from.SendMessage( "You pick {0} Grapefruit{1}!", pick, ( pick == 1 ? "" : "s" ) ); 
		...
		private class FruitTimer : Timer
		{
			...
			public FruitTimer( GrapefruitTree plant ) : base( TimeSpan.FromSeconds( 900 ), TimeSpan.FromSeconds( 30 ) )
			{
		...
		private class StumpTimer : Timer
		{
			...
			public StumpTimer( GrapefruitTree Tree, FruitTreeStump Stump, Mobile from ) : base( TimeSpan.FromMilliseconds( 5500 ) )
			{
		...
		public GrapefruitTree( Serial serial ) : base( serial )
		{
		}
