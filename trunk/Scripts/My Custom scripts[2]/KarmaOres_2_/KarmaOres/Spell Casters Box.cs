...
{
    public class SpellCastersBox : Item
    {
		...
		[Constructable]
        public SpellCastersBox() : base( 0xE80 )
        {
            Movable = true;
            Weight = 1.0;
            Hue = 518;
            Name = "Spell Caster's Box";
        ...
        [Constructable]
        public SpellCastersBox(int storageLimit, int withdrawIncrement) : base( 0xE80 )
        {
            Movable = true;
            Weight = 1.0;
            Hue = 518;
        ...
        {
            from.Target = new SpellCastersBoxTarget(this);
        }
        ...
        public SpellCastersBox(Serial serial) : base( serial )
        {
        }
	...
	public class SpellCastersBoxGump : Gump
    {
        private PlayerMobile m_From;
        private SpellCastersBox m_Key;
		
		public SpellCastersBoxGump(PlayerMobile from, SpellCastersBox key) : base( 25, 25 )
        {
            m_From = from;
            m_Key = key;

            m_From.CloseGump(typeof(SpellCastersBoxGump));
            ...
            AddLabel(200, 25, 88, "Spell Caster's Box");
			...
			AddLabel(325, 100, 0x486, "BlankScroll");
			AddLabel(425, 100, 0x480, key.BlankScroll.ToString());
			AddButton(275, 100, 4005, 4007, 20, GumpButtonType.Reply, 0);

			AddLabel(325, 125, 0x486, "Sand");
			AddLabel(425, 125, 0x480, key.Sand.ToString());
			AddButton(275, 125, 4005, 4007, 21, GumpButtonType.Reply, 0);

			AddLabel(325, 150, 0x486, "Beeswax");
			AddLabel(425, 150, 0x480, key.Beeswax.ToString());
			AddButton(275, 150, 4005, 4007, 17, GumpButtonType.Reply, 0);

			AddLabel(325, 175, 0x486, "Spring Water");
			AddButton(275, 175, 4005, 4007, 14, GumpButtonType.Reply, 0);
			if (m_Key.Regs.ContainsKey(typeof(SpringWater)))
				AddLabel(425, 175, 0x480, m_Key.Regs[typeof(SpringWater)].ToString());
			else
				AddLabel(425, 175, 0x480, "0");

			AddLabel(325, 200, 0x486, "Petrafied Wood");
			AddButton(275, 200, 4005, 4007, 15, GumpButtonType.Reply, 0);
			if (m_Key.Regs.ContainsKey(typeof(PetrafiedWood)))
				AddLabel(425, 200, 0x480, m_Key.Regs[typeof(PetrafiedWood)].ToString());
			else
				AddLabel(425, 200, 0x480, "0");
			
			AddLabel(325, 225, 0x486, "Destroying Angel");
			AddButton(275, 225, 4005, 4007, 16, GumpButtonType.Reply, 0);
			if (m_Key.Regs.ContainsKey(typeof(DestroyingAngel)))
				AddLabel(425, 225, 0x480, m_Key.Regs[typeof(DestroyingAngel)].ToString());
			else
				AddLabel(425, 225, 0x480, "0");

			AddLabel(325, 250, 88, "Each Max:" );
			AddLabel(425, 250, 0x480, key.StorageLimit.ToString() );	

            AddLabel(325, 275, 88, "Add resource or item");
            AddButton(275, 275, 4005, 4007, 22, GumpButtonType.Reply, 0);
        }
	...
	public class SpellCastersBoxTarget : Target
    {
        private SpellCastersBox m_Key;

        public SpellCastersBoxTarget(SpellCastersBox key) : base( 18, false, TargetFlags.None )
        {
            m_Key = key;
        }
