...
namespace Server.Items
{
	public class BlackSmithBoxGump : Gump
	{
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			if ( info.ButtonID == 16 )
			{
                if (m_Box.BlackSmith > m_Box.WithdrawIncrement)
                {
                    m_From.AddToBackpack(new SmithHammer(m_Box.WithdrawIncrement));
                    m_Box.BlackSmith = m_Box.BlackSmith - m_Box.WithdrawIncrement;
                    m_From.SendGump(new BlackSmithBoxGump(m_From, m_Box));
                }
				else if ( m_Box.BlackSmith > 0 )
				{
