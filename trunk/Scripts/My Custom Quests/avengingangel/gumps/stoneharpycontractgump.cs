...
{
	public class StoneHarpyContractGump : Gump
	{
		private StoneHarpyContract MCparent;
		
		public StoneHarpyContractGump( Mobile from, StoneHarpyContract parentMC ) : base( 0, 0 )
		{
			from.CloseGump( typeof( StoneHarpyContractGump ) );
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( info.ButtonID == 2 )
			{
				...
				m_from.AddToBackpack( new StoneHarpyClue() );
				m_from.AddToBackpack( new CharlesMarker7());
			}
		}
		...
		private class MonsterCorpseTarget : Target
		{
			...
			public MonsterCorpseTarget( StoneHarpyContract parentMC ) : base( -1, true, TargetFlags.None )
			{
				MCparent = parentMC;
			}
