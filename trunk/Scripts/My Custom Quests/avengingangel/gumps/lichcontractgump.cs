...
{
	public class LichContractGump : Gump
	{
		private LichContract MCparent;
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( info.ButtonID == 2 )
			{
				...
				m_from.AddToBackpack( new LichClue() );
				m_from.AddToBackpack( new CharlesMarker6());
			}
		}
		...
		private class MonsterCorpseTarget : Target
		{
			...
			public MonsterCorpseTarget( LichContract parentMC ) : base( -1, true, TargetFlags.None )
			{
				MCparent = parentMC;
			}
