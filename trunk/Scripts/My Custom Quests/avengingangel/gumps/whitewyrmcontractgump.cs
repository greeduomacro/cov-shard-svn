...
{
	public class WhiteWyrmContractGump : Gump
	{
		private WhiteWyrmContract MCparent;
		
		public WhiteWyrmContractGump( Mobile from, WhiteWyrmContract parentMC ) : base( 0, 0 )
		{
			from.CloseGump( typeof( WhiteWyrmContractGump ) );
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			if ( info.ButtonID == 2 )
			{
				...
				m_from.AddToBackpack( new WWClue() );
				m_from.AddToBackpack( new CharlesMarker());
			}
		}
		...
		private class MonsterCorpseTarget : Target
		{
			...
			public MonsterCorpseTarget( WhiteWyrmContract parentMC ) : base( -1, true, TargetFlags.None )
			{
				MCparent = parentMC;
			}
